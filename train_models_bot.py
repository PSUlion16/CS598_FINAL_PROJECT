...
model = fasttext.train_supervised(input="./cleansed_data/regular_input.train", dim=300, epoch=10, minCount=5, loss='ns')
...
result = model.test("./cleansed_data/regular_input.test")
my_logger.info(result)

# Train the rolled model - BOT
my_logger.info("Training the rolled BOT model ...")
model2 = fasttext.train_supervised(input="./cleansed_data/rolled_input.train", dim=100, epoch=3, minCount=5, lr=.003, loss='ns')
...
result2 = model2.test("./cleansed_data/rolled_input.test")
